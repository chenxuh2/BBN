...
TASK: Report which specific reflections actually occur in the conversation. Be STRICT and SPECIFIC:
   ...
   that is fine. Do NOT pad the list with weak, forced, or unsupported mappings; if there is no clear
   supporting quote, leave it out.
...
def run():
    ...
    COLUMNS = ["config", "model", "file", "goal", "item", "evidence", "reasoning"]
    ...
    for model in MODELS:
        ...
        for fp in tqdm(files, desc=model):
            ...
            if result is None:
                # parse failure -> goal="Error" (distinct from a legit "None" = no reflection found)
                print(f"  !! FAILED (bad/oversized JSON): {base} [{model}]")
                rows.append({"config": CONFIG_NAME, "model": model, "file": base,
                             "goal": "Error", "item": "Error", "evidence": "", "reasoning": ""})
                continue
            reflections = result.get("reflections", []) or []
            if not reflections:
                rows.append({"config": CONFIG_NAME, "model": model, "file": base,
                             "goal": "None", "item": "None", "evidence": "", "reasoning": ""})
                continue
