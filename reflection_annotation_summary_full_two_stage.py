...
def stage1_detect(transcript, model):
    prompt = f"""
...
TASK: Report each specific reflection in the conversation. Be STRICT: count a reflection only if a
   speaker explicitly evaluates/recalls/reflects on that action AND you can quote a direct supporting
   line. Exclude small talk, logistics, generic feelings.
...
OUTPUT (Strict JSON). Fill the keys in the given order so evidence/reasoning come BEFORE the code:
{{
    "reflections": [
...
def stage2_label(transcript, goal, item, evidence, model):
    prompt = f"""
...
Base your judgment PRIMARILY on the supporting evidence below; use the full conversation only as
background context.

FULL CONVERSATION (background context):
{transcript}

THE CONFIRMED REFLECTION:
  Goal: {goal}
  Item: {item}
  Supporting evidence (judge from this): {evidence}
...
def run():
    ...
    for model in MODELS:
        ...
        for fp in tqdm(files, desc=model):
            ...
            s1 = stage1_detect(transcript, model)
            if s1 is None:
                ...
                continue
            reflections = s1.get("reflections", []) or []
            if not reflections:
                rows.append({"config": CONFIG_NAME, "model": model, "file": base,
            ...
            for r in reflections:
                ...
                s2 = stage2_label(transcript, goal, item, ev, model) or {}
                rows.append({"config": CONFIG_NAME, "model": model, "file": base,
                             "goal": goal, "item": item,
